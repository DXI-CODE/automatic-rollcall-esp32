...
void imprimirdatosdetarjeta(int8_t hora, int8_t min, int8_t sec) {
  ...
  for (i = 7; i < 30; i++) {
    ...
    if (success) {
      ...
      lcd.setCursor(0, 1);
      lcd.print("de tarjeta RFID.");
      lcd.setCursor(0, 2);
      lcd.print("Vuelve a escanear");
      lcd.setCursor(0,3);
      lcd.print("tu tarjeta.");
      delay(6000);
      noerror = false;
      i = 27;
      } 
  ...
  for (int i = 0; i < 3; i++) {  // Up to 3 lines on LCD
  char aux[21] = { 0 };        // 20 characters max per LCD line
  int start = i * 20;

  if (start >= strlen(data2)) break;  // Stop if we're past the end

  strncpy(aux, data2 + start, 20);    // Copy up to 20 chars from data2
  aux[20] = '\0';                     // Null-terminate

  lcd.setCursor(0, i + 1);            // Line 1, 2, 3 (leave line 0 for time)
  lcd.print(aux);
  }
...
void chequeoHora(int8_t dia, int8_t hora, int8_t min, int8_t sec) {
  ...
  if (dia != 0 && dia != 6) {  //Se comprueba si hoy no es sabado o domingo
        ...
        (hora > horaInicio && hora < horaCierre))                                //Caso de hora actual sea xx:yy y la hora sea horaApertura < horaActual < horaCierre  (ej 12:15 y 11:30 - 13:00)
    {
      ...
      } else if (!reseteando){  //Caso de que se hayan acabado las clases para el area especifica
  ...
  } else if(!reseteando){  //Caso de dia sin clases (sabado o domingo)
            //Formateo de hora
    lcd.setCursor(0, 0);
    if (hora < 10 && min < 10) {
...
void servidor() {
  ...
  server.on(
    "/recibir", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
  ...
  void conseguirHoraBuena() {
  timeClient.begin();         // Ensure client is initialized
  timeClient.setUpdateInterval(60000); // Optional, once a minute
  timeClient.forceUpdate();   // Try fetching immediately

  unsigned long rawTime = timeClient.getEpochTime();

  int attempts = 0;
  while ((rawTime == 0 || rawTime > 2085956890UL || rawTime < 1600000000UL) && attempts < 10) {
    Serial.println("Esperando hora NTP válida...");
    delay(1000);
    timeClient.forceUpdate();
    timeClient.update();
  ...
  if (rawTime >= 1600000000UL && rawTime < 2085956800UL) {
    rtc.setEpoch(rawTime);  // Set RTC once valid time is retrieved
    Serial.print("Hora válida: ");
    Serial.println(rawTime);
    lcd.clear();
  } else {
    Serial.println("Fallo al obtener hora válida de NTP.");
    // Fallback: get time from RTC instead
  }
}
...
void setup() {
  ...
  archivo = (char *)malloc(sizeof(char) * TAMARCHIVO);

  //Inicializacion de la tarjeta SD y obtencion de JSON (clases)
  
  if (archivo == NULL) {
    Serial.println("Error al reservar memoria para el buffer");
    while (1)
      ;  // Detén ejecución
  }

  memset(archivo, 0, TAMARCHIVO);  // Limpia el buffer

  File dataFile2 = SD.open("/Lunes.json");
  if (dataFile2) {
    int i = 0;
    while (dataFile2.available() && i < TAMARCHIVO) {
      archivo[i++] = dataFile2.read();
    }
    archivo[i] = '\0';  // Cierre de cadena
    dataFile2.close();
  } else {
    Serial.println("Error al abrir Lunes.json");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Archivo Lunes.json");
    lcd.setCursor(0, 1);
    lcd.print("no encontrado.");
    while (1) delay(100);
  }

  DeserializationError error = deserializeJson(clases, archivo);
  if (error) {
...
void loop() {
  ...
  if (!reseteando) {
    ...
    if (WiFi.status() != WL_CONNECTED) {  //Si no hay internet
      ...
      if (timeClient.getEpochTime() < 2085956890) {
        ...
        if(!reseteando){  //No se limpiara la pantallasi se va a resetear el ESP32
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Recuperando la");
        lcd.setCursor(0, 1);
        lcd.print("hora...");
        conseguirHoraBuena();  //Funcion para obtener una hora aceptable que no sea en 2032
        }
